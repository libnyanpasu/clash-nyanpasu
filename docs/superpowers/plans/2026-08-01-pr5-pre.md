...
手写 v1.4.5 形状（= v2 结构去掉全部 v2-only 可选字段）。参考形状：
...
| architecture ledger 绿                   | S10            | `pnpm lint:architecture-ledger` exit 0（snapshot 已按预期 +1 重写并逐行审过）                                                                                                      |
...
| ledger snapshot 不变                                  | **有一处预期偏差**（marker +1），见 S10；需 leader 在 review 时确认接受 |
...
| v1 fixture 形状与真实 v1.4.5 输出有出入                          | 中（无 v1 源码可比对）          | T2/T3 证明力打折              | T1 断言 fixture 不含任何 v2-only 键，保证它至少是**严格子集**；这已足以支撑「子集必然解码成功」这一论点                      |
...
| `nyanpasu-core-manager` / `clash-api` workspace 条目 | **PR-5a**（决策点 D1 选项 A）                                                                                                                      |
