...
mod research_probes {
    ...
    fn entry(line: u64, timestamp: u64) -> LogEntry {
        LogEntry { line_number: line, level: LoggingLevel::INFO, timestamp,
            target: "probe".into(), start_pos: 0, end_pos: 0 }
    }

    #[test]
    fn reproduces_duplicate_timestamp_loss() {
        let mut index = LogIndex::new();
        index.add_entry(entry(0, 1000));
        index.add_entry(entry(1, 1000));
        let results = index.query(QueryBuilder::default().timestamp(1000..1001).build().unwrap()).unwrap();
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].line_number, 1);
    }

    #[test]
    fn reproduces_out_of_order_timestamp_false_positive() {
        let mut index = LogIndex::new();
        for (line, timestamp) in [1000, 9000, 2000].into_iter().enumerate() {
            index.add_entry(entry(line as u64, timestamp));
        }
        let results = index.query(QueryBuilder::default().timestamp(1000..3000).build().unwrap()).unwrap();
        assert_eq!(results.len(), 3);
        assert!(results.iter().any(|row| row.timestamp == 9000));
    }

    #[tokio::test]
    async fn reproduces_partial_append_resets_committed_offset() {
        let mut file = tempfile::NamedTempFile::new().unwrap();
        writeln!(file, "{}", r#"{"level":"INFO","target":"probe","timestamp":"2026-09-11T00:00:00Z"}"#).unwrap();
        file.flush().unwrap();
        let mut indexer = Indexer::new(Utf8PathBuf::from_path_buf(file.path().into()).unwrap());
        indexer.build_index().await.unwrap();
        let old_offset = indexer.current.end_pos;
        assert!(old_offset > 0);
        file.write_all(br#"{"level":"INFO","target":"#).unwrap();
        file.flush().unwrap();
        assert!(indexer.on_file_change().await.is_err());
        assert_eq!(indexer.current.end_pos, 0);
        assert_eq!(indexer.current.line, 0);
    }

    #[test]
    fn reproduces_trace_parse_rejection() {
        let row = r#"{"level":"TRACE","target":"probe","timestamp":"2026-09-11T00:00:00Z"}"#;
        assert!(serde_json::from_str::<TracingJson>(row).is_err());
    }

    #[tokio::test]
    async fn measure_build_and_incremental_index() {
        let mut file = tempfile::NamedTempFile::new().unwrap();
        for i in 0..100000 {
            let timestamp = chrono::DateTime::from_timestamp_millis(1789084800000 + i).unwrap();
            writeln!(file, "{{\"level\":\"INFO\",\"target\":\"probe::module\",\"timestamp\":\"{}\",\"fields\":{{\"message\":\"sample log entry {}\"}}}}", timestamp.to_rfc3339(), i).unwrap();
        }
        file.flush().unwrap();
        ...
        assert_eq!(indexer.current.line, 100000);
        ...
        file.flush().unwrap();
        let start = std::time::Instant::now();
        indexer.on_file_change().await.unwrap();
        let increment = start.elapsed();
        assert_eq!(indexer.current.line, 101000);
        println!("PROBE: bytes={bytes}, initial_rows=100000, initial_ms={:.3}, appended_rows=1000, append_ms={:.3}", build.as_secs_f64()*1000.0, increment.as_secs_f64()*1000.0);
    }
}
