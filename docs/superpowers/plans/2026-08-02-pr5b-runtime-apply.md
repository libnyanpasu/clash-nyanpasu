...
| T-PC-14  | legacy clash mirror 持久化失败                     | `patch_clash_with_rebuild` 成功收尾 seam       | typed commit + rebuild 已成功后，in-memory mirror 仍 apply；注入的 `save_config` 失败只写 error log，返回保持 `Ok(rebuilt)`，不得被翻转成 `Err`                                                                                                                                                                                                                              |
