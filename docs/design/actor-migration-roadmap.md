...
| PR-4 runtime 派生化          |       ✅ |     ❌ 未闭环 | 换核锁域、rollback store、Applied 状态、candidate/path、真实目录测试、smoke evidence       |
| **PR-4S 稳定化门**           |       ⏳ | 进行中/未完成 | S01 基线文档已刷新至 `9886aacc`；fixtures/ledger 与 S02～S10 尚未完成；不得宣告 PR-4S 完成 |
...
| S-R2  | PR-4   | 深层 rollback 恢复产品文件但未恢复 runtime read model                                          | PR-4S                                |
