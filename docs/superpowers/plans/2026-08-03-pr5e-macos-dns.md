...
**中途更正（leader 在 v2 撰写期间送来，逐条处置）：**

| 更正                                                                     | v2 的处置                                                                                                                                                                                                                                                                                                |
| ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 预算写成 `READ + WRITE + IPC` 三常量之和会**低估**；应是对算法的遍历计数 | **本 PR 独立推导出同一个 `4 ×`**（路径不同：leader 经 `can_address` 计一次读，本文档已删该方法，那次读改由 Service 写内部的预检承担）。§3.3 给的是**公式 + 逐项推导 + 改动义务**，不是冻结的三项之和                                                                                                     |
| 文档里没有任何 `ACTOR_STOP_BUDGET`；5d 单方面写"5e 须复核"不构成契约     | **对 v1 成立，对 v2 已不成立**（§1.2/§3.3/§9.1/§10 均已出现）。**但确实缺一张带 owner 的互指表**——已补 **§3.3.1**，形式按 5d v7 §2.5 原则 D                                                                                                                                                              |
| R-C2-7 不是合法的"填槽"：要改冻结槽位之外的代码                          | **同意，且本 PR 本来就在改。** 现显式声明为 **S4**（§1.2 说明框 + §4.8.2），改动逐行写出；**5d v8 无论收编与否都能承接**                                                                                                                                                                                 |
| `stop(None)` 的撤回依赖 `CoreClientInner::drop` 是否在退出时触发，未确立 | **两轮**：v2 先写成"两个分支都不救 S3"的析取式（§9.4）；5d v8 随后把该问题**确立为否定事实**（`Drop` 在任何已发布退出路径上都不触发），§9.4 据此塌成单分支论证，并**独立复核了三条 vendored 锚点**、补上 5d 枚举里缺的 restart 路径。R-C3-6 措辞从"可能没跑"收紧为"不会执行"，**现由事实而非析取式支撑** |
...
| **R-C2-7**：5d §4.4 第 2/4/6 行（收敛失败）到不了 S2，本 PR 须让它**可见降级而非静默**                                                                | §4.7 S2 逐行核对、R-C2-7  | **§4.8.2**（**S4，见下**）+ T-DNS-37                     |

> ### 关于第 3 条：它**不是**「填槽」，本文档不这么声称
>
> 5d v7 §4.7 把接缝描述成「槽位与前置条件由 5d 冻结，**槽内内容**由 5e 填」。**对 R-C2-7 而言这句话是假的**：`apply_mode` 的失败路径在 `?` 处提前返回，**根本不经过 S2 槽位**；要让它可见降级，本 PR 必须改动**冻结槽位之外**的代码。
>
> **本 PR 因此自行声明第四个触点，命名为 S4**（§4.8.2），并把它改动的每一行写出来。**若 5d v8 决定把它收编为正式接缝（例如 `SEAM-5E-S4`），本文档的 S4 直接改名即可，位置与内容不变；若 5d v8 改为收窄「只填槽」那句话，则 S4 保持为本 PR 自有的改动。两种收场本文档都能承接。**
>
> **不写这一条的代价是具体的**：5d 的审查者会按「5e 只填槽」去核对 `apply_mode` 的 diff，然后发现多出一个内联 `async` 块和一处 `publish`，而没有任何文档说过它会出现。
...
**这个 `4 ×` 不是常量，是对 §4.4 算法的一次遍历计数。** 写成三个常量之和（`READ + WRITE + IPC`，v1 与本 PR 任务书最初的说法）会**低估**：那只数了"一次读、一次本地写、一次 IPC 写"，漏掉了**循环遍历两个适配器**、**每次写后都有强制回读**、以及 **Service 写内部还有一次默认设备预检**。

**因此本节给的是公式，不是数字，并附一条改动义务：**

> **凡改动 §4.4 的候选循环形状——增删适配器、增删回读、加重试、把归属判定挪进循环——都必须回到本节重新推导 `DNS_RESTORE_BUDGET`，并重新核对下面的不等式。** 判据：`DNS_RESTORE_BUDGET` 的定义处必须紧邻那段逐项推导；`rg -n 'DNS_RESTORE_BUDGET'` 的每一处命中都能追到这一节。
...
5d v7 §2.5 的原则 D 要求：**跨文档契约的每一处，两份文档必须能互相指到对方；单方面冻结的槽位不算契约。** 5d 在**它的**常量表 owner 列写了"PR-5e 必须复核"，那只是它那一侧的一半。**本表是另一半。**

| 项                          | owner     | 内容                                                                               | 怎么核                                               |
| --------------------------- | --------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------- |
| `DNS_READ/WRITE/IPC_BUDGET` | **PR-5e** | 逐次 I/O 预算，实测取上界（§3.2）                                                  | 实施报告记实测依据                                   |
| **`DNS_RESTORE_BUDGET`**    | **PR-5e** | **公式** `4 × READ + WRITE + IPC`，推导见上；由 `Shutdown` 臂的外层 `timeout` 强制 | T-DNS-38（行为）；改动义务见上框                     |
| `ACTOR_STOP_BUDGET`         | **PR-5d** | 5d §4.3 常量表；owner 列已注明"PR-5e 必须复核"                                     | **本节不等式**。5e 侧的复核结论须写进 PR 描述（§10） |
| 不等式本身                  | **两者**  | `ACTOR_STOP_BUDGET ≥ DNS_RESTORE_BUDGET + backend.shutdown() 上界 + 余量`          | **两份 PR 描述都要出现**；任一侧单独写不构成契约     |

**为什么必须是不等式而不是"注意一下"**：若它不成立，关停不是**偶尔**超时，而是**每次**超时——于是 5d §4.6.3 的 `AbandonedUnverified` 变成**默认路径**，后端清理与本 PR 的 S3 恢复**双双稳定跳过**。**一次预算失配把边缘情形变成主路径。**
...
### 4.8 收敛尾部：S2（重施加，槽内）与 S4（收敛失败的可见降级，**槽外**）
...
#### 4.8.2 S4 —— 收敛失败（5d R-C2-7）：不重施加，但**必须可见**

> **本节声明的是一个\*\*槽位之外\*\*的触点。** 5d v7 冻结的 S2 位于 `apply_mode` 的**成功**尾部；收敛失败在 `?` 处提前返回，**到不了那里**。因此「5e 只填槽」对本节不成立——本 PR 在此改动 5d 交付的函数体本身。**称它为 S4，把改动逐行写出来，等 5d v8 决定是否收编。**
...
| **R-C3-6** | **关停超时 ⇒ S3 恢复不会执行**（措辞按 §9.4 收紧：不是"可能没跑"，而是"跑到属于机会主义，三个条件缺一即不跑"）。5d 已撤回 `stop(None)`，且**不留 facade 侧 S3 槽位**，因此**没有第二次机会**    | **v2 新增；5d R-C2-4 在本 PR 的对应面。** 其"慢、随后返回则 `Shutdown` 照常执行"的前提**已被证伪**：`CoreClientInner::drop` 在任何已发布退出路径上都不触发（§9.4 三条锚点） | 用户可见后果与指引同 R-C3-1。移除条件 = PR-6 启动时清理；或后端 await 各自有界（5a 设计面）。**该残留现由事实支撑，不再依赖任何未决问题**                         |
...
### 9.4 关停超时后 S3 到底会不会跑 —— **只剩一条不受约束的机会主义路径**

5d §4.6.3 用一张逐案表论证撤回 `stop(None)` 更优，其中「处理器只是**慢**、随后返回」那一格写的是：

> 「`Shutdown` 照常执行 → 后端清理与 5e 的 DNS 恢复都跑到」

**这一格断言的是三个条件的合取，而 5d 只确立了第一个。** 逐条核实：

| 条件                                                   | 状态                                                                                                                                                                                                                                                                                               |
| ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| (a) 卡住的处理器**确实会**返回                         | 5d 的分案前提，**该格内成立**                                                                                                                                                                                                                                                                      |
| (b) 它在**进程退出之前**返回                           | **未确立，且不受任何约束。** 生产入口是 `utils/help.rs:249` 的 `cleanup_processes`：`:263` `client.shutdown().await` 返回后，还有 `:265` `widget_manager.stop().await` 与 `:270`，随后 `quit_application`（`:274`）调 `app_handle.exit(0)`。**窗口存在但长度不受约束**，也没有任何机制让它等 actor |
| (c) 在此期间**没有** `stop(None)` 抢在 `Shutdown` 之前 | **已确立为「不会发生」**（5d v8 追进 vendored 源码，本文档独立复核，锚点见下）。`CoreClientInner::drop` 就是 `self.actor_ref.stop(None)`（`client/core.rs:377-380`），**但它在任何已发布的退出路径上都不会被调用**——进程在 Tauri 托管状态被析构之前就退出了                                        |

**(c) 的三条锚点（5d v8 提出，本文档逐条独立打开复核，非转述）：**

| 路径                                | 锚点                                                                                                                                               | 结论                                       |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| `app.run(..)` 正常退出              | `tao-0.35.3/src/platform_impl/windows/event_loop.rs:230` 与 `macos/event_loop.rs:202`：`run<F>(..) -> !` 结尾是 `process::exit(exit_code)`         | `App` 与托管状态**从不被析构**             |
| Tauri 的 `RuntimeRunEvent::Exit` 臂 | `tauri-2.11.5/src/app.rs:1430-1437` 只调 `cleanup_before_exit()`；该函数（`:1108-1120`）清的是 tray icons 与 `resources_table()`，**不碰托管状态** | 托管状态的 `NyanpasuClient` 不在清理范围内 |
| `restart_application`               | `utils/help.rs:295` 直接 `std::process::exit(0)`；Tauri 侧的 restart 分支（`app.rs:1436` → `process.rs:74-89`）同样以 `exit(0)` 收尾（`:88`）      | **不运行任何析构函数**                     |

> **本文档补一条 5d 的枚举里没有的路径**：`app.rs:1436` 的 `restart_on_exit` 分支走 `crate::process::restart`，其 `:88` 也是 `exit(0)`。**这不改变结论，只是把"已发布的退出路径"这个全称量词的范围补齐**——按 5d §2.4 的方法论，下断言前要先把量词范围定死。

**结论——`Drop` 不再是悬而未决的问题，(c) 是已确立的否定，论证从两分支塌成一条：**

`CoreClientInner::drop` 在两条退出路径上都不触发 ⇒ **没有任何东西终止 actor** ⇒ 进程从它底下退出 ⇒ **排队的 `Shutdown` 不会被处理，S3 不执行。**

**唯一的例外**是处理器在 (b) 那个**不受约束**的窗口内返回。**那是机会主义，不是保证**——窗口长度没有任何机制约束，也没有任何机制让退出去等它。

**因此 R-C3-6 的措辞按"不会执行"写，而不是"可能没跑"，且该措辞现在由事实支撑，不再由一个析取式支撑。** 本 PR 不把任何正确性挂在这条路径上；用户可见的兜底是 R-C3-1 的手动指引，机制上的兜底是 PR-6 的启动时清理。

> **反向的一个副产品，记在这里免得将来被当成新发现**：既然 `CoreClientInner::drop` 在退出路径上不触发，5d §4.6.3 那句「actor 的终止并不依赖 `stop(None)`：`CoreClientInner::drop` 本来就会 `stop(None)`，在 client 被丢弃时收尾」**在退出场景下也不成立**。它在**非退出**场景（client 被真正丢弃而进程继续运行）下仍然成立——而那正是 R-C2-4 说「关停可接受**仅因为进程随后就退出**，该前提在『重启 actor 而不退进程』的用法下不成立」所指的那一类。**两句话指的是同一条分界线，方向相反，都要保留。**

> **这一条回递给 PR-5d**：5d §4.6.3 那张表的"慢，随后返回"格**措辞强于机制**（正是它自己 §5「第二遍」要抓的那类）。它想论证的结论（撤回 `stop(None)` 不比发它更差）**依然成立**——因为发 `stop(None)` 在同样三格里没有一格更好——但**支撑它的那句"清理都跑到"应当收窄为"清理保留了执行的机会"**。**本节的存在不构成对 5d 裁定的反对，只反对那一格的措辞。**
...
| **`ACTOR_STOP_BUDGET` 的预算义务已兑现**                                                           | §3.3：`DNS_RESTORE_BUDGET` 由**公式**给出（不是冻结的常量之和）且由 `Shutdown` 臂的外层 `timeout` 强制；5d 只需核对一个不等式。**须写进 PR 描述**                                                                          |
| **跨文档预算契约的\*\*两半\*\*都在**                                                               | §3.3.1 互指表（owner + 公式 + 怎么核）；**该不等式必须同时出现在 5d 与 5e 两份 PR 描述里**——任一侧单独写不构成契约（5d v7 §2.5 原则 D）                                                                                    |
| **改动 §4.4 候选循环时重新推导了预算**                                                             | §3.3 的改动义务框；`rg -n 'DNS_RESTORE_BUDGET'` 的每一处命中都能追回 §3.3 的逐项推导                                                                                                                                       |
...
| **S4 已声明**：本 PR 在 `apply_mode` 的**失败路径**上改了 5d 交付的代码，「5e 只填槽」对该处不成立 | §1.2 说明框 + §4.8.2；**须写进 PR 描述**。5d v8 若把它收编为正式接缝，本文档改名即可                                                                                                                                       |
| **对 5d `apply_mode` 的改写不破坏 G-SEAM-02 / T-SEAM-02**                                          | §4.8.2 末尾的逐条核对；**须写进 PR 描述**（5d 的审查者不应自己去发现）                                                                                                                                                     |
| **关停超时后 S3 不执行**（由事实支撑：`CoreClientInner::drop` 在任何已发布退出路径上都不触发）     | §9.4 的三条件核实 + 三条 vendored 锚点（本文档独立复核，非转述 5d v8）；R-C3-6 措辞已按此收紧。**回递给 5d 的两条措辞修正在 §9.4 末**                                                                                      |
