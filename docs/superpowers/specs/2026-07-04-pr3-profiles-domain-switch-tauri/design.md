...
## 9. IPC 全量 BC 切换(13 → 16 条;T09 补口 = 17 条)
...
| (新增,T09 plan 补口)               | `set_profile_valid_fields`                                                                      | `set_profile_valid_fields(Vec<String>)`                                         | clash 字段白名单写入口(actor `SetValidFields`,`AffectsRule::Always` → 恒 rebuild);前端 `field-filter-card` 消费(旧 `patch_profiles_config({valid})` 载荷的去处) |
