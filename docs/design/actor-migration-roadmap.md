...
**日期：** 2026-07-13  
**状态：** Implementing（S01～S07 已完成；S08～S10 pending；不得宣告 PR-4S 完成）
...
| ---------------------------- | -------: | ------------: | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| PR-1 `NyanpasuClient` facade |       ✅ |   ⚠️ 条件通过 | process-global rebuild bridge、不可重装测试图、结果语义不完整                                                                                        |
...
| PR-3 profiles 域切换         |       ✅ |   ⚠️ 条件通过 | S07 已补齐 profile 文件/状态事务与 crate-internal degradation；warning → 公共 `MutationOutcome` wire 仍待 S08；已发生回归的 contract tests 继续固化  |
| PR-4 runtime 派生化          |       ✅ |     ❌ 未闭环 | S03/S04 已补齐 promoted/applied + 统一 lifecycle lease；S05 已补齐 D6 Applied compensation；仍待 dispatcher/fake-core（S09）与 smoke evidence（S10） |
| **PR-4S 稳定化门**           |       ⏳ | 进行中/未完成 | S01～S07 已完成；S08～S10 pending；不得宣告 PR-4S 完成。S09 的 `REGEN_BRIDGE` two-client isolation contract 是唯一稳定的 full-suite red contract。   |
...
| S-R5  | PR-1/4 | process-global `REGEN_BRIDGE` first-install-wins，通道无界，测试图不可重装                     | PR-4S；最终删除在 PR-5/6                                                                                                                         |
| S-R6  | PR-3   | profiles 状态先提交、文件后操作；warning 仅日志，调用方可能得到 `Ok`                           | PR-4S / S07 已关闭事务与 crate-internal degradation；公共 wire 留给 S08                                                                          |
| S-R7  | PR-3   | remote refresh 文件已更新而元数据 persist 失败时缺少文件恢复                                   | PR-4S / S07 已关闭（file-first promote → state CAS → complete/compensate）                                                                       |
...
- S06 tests 以 oneshot/mpsc channel 与 release barrier 固定交错，不使用 sleep；prepare 零提交、manager CAS、第二/第三域失败、逆序补偿、并发 conflict、finalizer uncertainty 与 structured `PartialCommit` 均已验证。S08～S10 仍 pending。
...
- 协议固定：
  - **state-first**：`prepare(next) → state CAS → promote → complete/compensate`（Add、ReplaceDefinition、Delete 的 cleanup 准备阶段）；
  - **file-first**：`prepare(next) → promote → state CAS → complete/compensate`（Remote refresh、External Mirror 同步）；
...
- 操作映射：Add / ReplaceDefinition → state-first（slot 变化时附带 resource；旧 path 可附 cleanup）；Delete → state commit 为权威 + durable cleanup；RefreshRemote / External Mirror → file-first；`ReconcileMaterializations` 串行执行 recovery。
...
- `ProfileDegradation{phase,code,message,retryable}` 仅 crate-internal（`Cleanup`/`Reconcile` × `JournalInvalid`/`MaterializationDeferred`/`CleanupDeferred`）。公共 `MutationOutcome` / IPC / Specta / frontend 仍完全归属 **S08**，尚未开始。
- S07 failure/crash/cleanup/fence tests green；S08～S10 仍 pending。
...
- profile add/refresh 的文件故障有确定 compensation；
- 全部自动测试零访问真实用户配置目录；
