...
impl MigrationStore {
    ...
    pub fn flush_atomic(&self, path: &Path) -> anyhow::Result<()> {
        ...
        atomic_write(path, &bytes).context("failed to persist migration state")
    }
...
/// Atomically write `contents` to `path` via a sibling temp file plus a durable
/// rename. On a crash mid-write the destination is never left half-written —
/// only the `<file_name>.tmp` sibling can be partial, while the original stays
/// intact. Used for both the migration state store and the user config files
/// each migration step rewrites.
...
mod tests {
    ...
    fn atomic_write_replaces_existing_store() {
        ...
        assert!(!temp.path().join(format!("{STORE_FILE_NAME}.tmp")).exists());
    }
