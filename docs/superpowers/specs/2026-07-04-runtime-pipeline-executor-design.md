...
| `filter__x`                | 目标须存在且为序列。过滤值：字符串→Lua 谓词逐项 `retain`（求值错误 = 移除该项，parity）；序列→递归多过滤器；映射 `when`+`expr`→真值项替换为 `eval_item_expr` 结果；`when`+`override`→字面替换;`when`+`merge`→逐键深合并（动作按旧 arm 顺序 expr→override→merge→remove 带类型卫选择：`merge` 非映射不匹配本动作、落入后续动作，全部不匹配 = 单次「invalid filter」WARN 且不逐项求值 `when`；命中项非映射 → WARN 保留原项，旧代码 panic，见 §13 #15）；`when`+`remove`→按点路径/索引删除（字符串路径仅对映射项生效、数字索引仅对序列项生效，非法形状 → WARN 保留，parity） |
...
全局 transform 变更时，即使 `current == None`（bare 模式）也返回 `FullCurrent`——bare 管线同样执行 global transforms（§13 #9），缓存的 bare artifact 不得过期。`affected_configs` 此时为空（无选中 Config 可报告），但重建信号必须发出。
...
| 15  | `filter__` merge 命中非映射项       | **panic**（`merge.rs:163` `as_mapping_mut().unwrap()`） | WARN + 保留原项                                             | 崩溃不是语义（同 #3）                                                     |
...
以下三条为 2026-07-05 多模型审查后的修正（§7.3/§11/§13 正文已同步更新）：

4. `filter__` 动作选择重构为旧 arm 顺序 + 类型卫（expr→override→merge(映射)→remove(序列)）：`merge` 非映射不匹配本动作、按旧代码落入后续动作（如 `remove`），全部不匹配 = 单次「invalid filter」WARN 且不逐项求值 `when`；`merge` 命中项非映射 → WARN 保留原项（旧代码 panic，登记为 §13 #15）。
5. `filter__` `when`+`remove`：字符串路径仅对映射项生效、数字索引仅对序列项生效（对齐 `merge.rs:186/221`；此前实现把字符串 `"0"` 当根级数组索引删除）。
6. `invalidate_profile`：全局 transform 变更在 `current == None`（bare 模式）时也返回 `FullCurrent`——bare 管线同样执行 global transforms，缓存的 bare artifact 不得过期（§11）。
