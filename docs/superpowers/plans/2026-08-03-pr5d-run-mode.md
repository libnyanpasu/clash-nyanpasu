...
| §4.6.3「不发 `stop(None)` ⇒ 清理与 DNS 恢复**都跑到**」实为三条件合取，v8 只建立了 (c)（**由 PR-5e 规划者指出**） | **收窄为「保留了执行的机会」**：(b)「赶在进程结束前返回」的窗口（`help.rs:263→265→270`）无任何机制约束；并写明 (c) 两个分支殊途同归——`Drop` 若触发则 `Shutdown` 被抢占、不触发则 actor 被 `process::exit` 连根拔掉。**撤回 `stop(None)` 的结论不受影响**。同时把 §5 第二遍**扩为跨文档**（须从消费方立场复读己方措辞），并使 R-C2-4 与 5e 的 R-C3-6 相容 |
...
| 预算义务写成三个常量之和                                                                                          | **改为对 5e 算法的遍历计数**；**修订期间 5e v2 并发落地并以更好的形状兑现了它**——5e §3.3 定义单一常量 `DNS_RESTORE_BUDGET` 并用外层 `timeout` 强制，本 PR 义务因此收敛为 `ACTOR_STOP_BUDGET ≥ DNS_RESTORE_BUDGET + backend.shutdown() 上界 + 余量`（§4.7）。**v8 初稿那句「5e 全文零命中 `ACTOR_STOP_BUDGET`」已作废并更正**（§2.5 D5）                  |
...
8. **为 PR-5e 声明四个接缝**（§4.7，S1–S4）——**槽位与前置条件在本 PR 冻结**，槽内内容由 5e 填；
...
范围 = §4.7 的四个接缝 + 跨文档预算义务。**这条原则是 v6 那次失败的直接产物**——v6 单方面冻结了 S2/S3，而 PR-5e 的 §4.6/§4.8 早已把 owner 与位置定在别处，**两份文档各自内部自洽、合起来无法实施**。检索：`rg -n 'PR-5e|pr5e|SEAM-5E' docs/superpowers/plans/2026-08-03-pr5d-run-mode.md` 与 `rg -n 'PR-5d|pr5d' docs/superpowers/plans/2026-08-03-pr5e-macos-dns.md`，**逐条配对**。
...
| D4  | S4 收敛失败尾                | §4.7「这就要求第四个槽位」段       | 5e v2 已引用 `apply_mode` 与 S4                                                                                     | **v8 与 5e v2 并发落地后一致**；v1 时期 5e 无此概念                           |
| D5  | `ACTOR_STOP_BUDGET` 预算义务 | §4.3 owner 列 + §4.7 单常量关系    | 5e v2 §3.3 定义 `DNS_RESTORE_BUDGET` 并用外层 `timeout` 强制；§3.3.1 给出带 owner 的互指表；R-C3-6 对接 R-C2-4      | **一致**——但本行刚被更正过，见下                                              |

> **D 表的价值既在于它把不一致显示出来，也在于它\*\*会自己过期\*\*。** D4/D5 在 v8 初稿写下时确实不一致（当时 5e 还是 v1），几小时后就因 5e v2 并发落地而变成一致——**这说明对照表必须在每次修订时\*\*重跑检索\*\*，不能抄上一版**。本轮正是靠重跑 `rg -c ACTOR_STOP_BUDGET <5e 路径>`（得 13，非 0）抓到自己这两行已经过期。
...
| 慢，随后返回              | **抢在 `Shutdown` 前终止 → 清理\*\*必定\*\*被跳过** | 清理**保留了执行的机会**（**不是「一定跑到」**——见下方三条件，R-C2-4） |
| 已经死了（`SenderError`） | no-op                                               | no-op                                                                  |

**没有任何一格是 `stop(None)` 更好的。** ——**这个结论不依赖清理是否真的跑到**，见下一段。

##### 第二行「不发」列曾写成「清理都跑到」，那是一个三条件合取，而只有一条被建立（PR-5e 规划者提出）

「`Shutdown` 照常执行 ⇒ 后端清理与 DNS 恢复都跑到」要求**同时**成立：

| 条件    | 内容                            | 状态                               |
| ------- | ------------------------------- | ---------------------------------- |
| **(a)** | 卡住的处理器**确实会返回**      | 本行自身的前提，姑且成立           |
| **(b)** | 它在**进程结束之前**返回        | **未建立，且无机制约束**——见下     |
| **(c)** | 期间没有任何东西发 `stop(None)` | **已建立为成立**（本节九条事实链） |

**(b) 的窗口是无约束的**：`cleanup_processes`（`utils/help.rs:249`）在 `:263` 的 `client.shutdown().await` 返回之后，还剩 `:265` 的 `widget_manager.stop().await` 与 `:270`，随后处理器返回、Tauri 走向 `process::exit`。**这一段既没有等 actor，也没有任何东西让它去等。** 处理器能不能赶在进程消失前跑完 `Shutdown`，是一场**无界竞速**。

**关键之处在于 (c) 的两个分支殊途同归——这一点反直觉，必须写下来：**

- 若那个 `Drop` **会**触发 → `stop(None)` 抢在排队的 `Shutdown` 前面 → **清理被跳过**；
- 既然它**不**触发 → 就**没有任何东西**终止 actor，进程随后 `exit` 把它连根拔掉 → **清理同样被跳过**，除非处理器恰好赶在 (b) 那个无约束窗口内返回。

**所以清理是「有机会跑」，不是「会跑」。** 撤回 `stop(None)` 的结论**不受影响**——三格里发它都不更好，而九条事实链给了它第三条独立理由；**站不住的只是那句「都跑到」**。措辞已按此收窄，并指向 **R-C2-4**。

##### 上表第二行还依赖另一个前提，v7 没有查证它 —— **v8 已经查证，逐级引用如下**
...
#### 4.6.4 单次飞行：leader / follower 选举 + **两维状态**（完成 vs 放弃）
...
**PR-5e v2 已经兑现了这条义务，而且形状比上表更好**（`plans/2026-08-03-pr5e-macos-dns.md` §3.3，与本节并发落地）：**它不要求 5d 去累加一串 5e 的常量，而是把那个和收敛成一个由 5e 拥有、5e 实测、并在 `Shutdown` 臂里用真实外层 `timeout` 强制的单一常量。**

```text
（PR-5e §3.3 定义、拥有、实测）
DNS_RESTORE_BUDGET  ≜  4 × DNS_READ_BUDGET + DNS_WRITE_BUDGET + DNS_IPC_BUDGET

（本 PR 的义务因此收敛成一条单常量关系）
ACTOR_STOP_BUDGET   ≥  DNS_RESTORE_BUDGET
                     + backend.shutdown() 的正常上界（§4.3 本行原有依据）
                     + 调度余量
```

**为什么这个形状更好**：跨 PR 耦合从「5d 必须跟踪 5e 算法的每一次改动」降为「5d 只需覆盖 5e 的一个具名常量」。5e 的候选循环再怎么改，重新推导 `DNS_RESTORE_BUDGET` 是**5e 自己的事**，本 PR 的判据一个字不用动。**这才是 §2.5 原则 D 想要的那种可双向核对的契约。**

> **两侧独立推出同一个 `4 ×`，但归属不同，记一笔以免下次对不上**：上表把其中一次读记在 `can_address` 上；**5e v2 已把 `can_address` 从 trait 删除**（其 §3.1 注：候选排序恒等于常量 `[Local, Service]`，该方法在恢复路径上唯一的调用点随之消失），那次读改由 `ServiceMacosDns::write` 内部的漂移预检承担。**计数相同、归属不同——以 5e §3.3 的逐项推导为准，上表仅保留为本 PR 当初独立推导的记录，不再自行维护第二份。**

若该式不成立，关停会**稳定**超时、**稳定**走进 §4.6.3 的 `AbandonedUnverified`，从而**稳定**跳过清理与 DNS 恢复。**5e v2 已为这一情形单列残留 R-C3-6**（其 §9.3），与本 PR 的 R-C2-4 对接；其 §3.3.1 也已补上带 owner 的双向互指表。**v8 初稿写的「5e 全文零命中 `ACTOR_STOP_BUDGET`」对 v1 成立、对 v2 已不成立，此处按 v2 更正。**
...
>
> > **v8 的第二遍在文档内部跑过一次，仍然漏了一条**——§4.6.3 那格「清理与 DNS 恢复**都跑到**」。它是一个三条件合取（(a) 处理器返回、(b) 赶在进程结束前返回、(c) 无人发 `stop(None)`），而 v8 只建立了 (c)。**漏检的原因是结构性的**：过强的是**本文档向 5e 作出的承诺**，从本文档内部看它只是一句结论，**只有站在消费方的位置上才看得出它承诺了什么**。**由 PR-5e 的规划者指出。**
> >
> > **因此第二遍自 v8 起扩为跨文档**：§2.5 原则 D 列出的每一处跨文档契约，都要**从对方文档的立场**再读一遍己方措辞——问「对方会据此假定什么？我真的保证得了吗？」。**Principle D 建立了双向引用，第二遍就必须双向跑**，否则它只覆盖得到本文档自己的散文。
>
...
| **R-C2-4** | **`ACTOR_STOP_BUDGET` 超时后，后端清理是否执行**未知\***\*：`backend.shutdown()`（`core/actor/mod.rs:609`，全仓唯一调用点）可能没跑；Service 模式下 daemon 是独立进程，**进程退出不会带走它管的核**。5e 落地后被跳过的还包括 DNS 恢复。**已在一条已发布代码路径上实际成立\*\*：`restart_application`（`utils/help.rs:279-296`）先 `cleanup_processes`（`:280`）、再 `spawn` 后继进程（`:290`）、再 `process::exit`（`:295`）——超时时旧核可能与新实例并存 | **既有**（ractor 不抢占执行中的 `handle()`，本 PR 未引入也无法消除）。本 PR 把**等待**从无界改为有界，并把结果**如实报成降级关停**而非干净关停。**清理能否补跑取决于一场无界竞速**（§4.6.3 的三条件 (a)/(b)/(c)），因此**任何一方都不得把正确性建立在它跑到之上** | 「进程随后销毁」已由事实链 6/7/9 确证，但**销毁 ≠ 清理已发生**，且 `restart_application` 上还有后继进程紧接着启动。**与 PR-5e 的 R-C3-6 相容**：本条记的是认识论状态（**未知**），5e 记的是工程处置（**按「S3 不会跑」设计，不依赖 (a)/(b)/(c) 任何一条**）——「未知」与「按最坏情形设计」不矛盾，**两者合起来才是完整的账**。移除条件 = 后端 await 各自有界（属 PR-5a 设计面）或 ractor 提供处理器抢占 |
