...
by release version; an unreleased source commit is only a review dependency.
The runtime PR prepares service/runtime version `2.0.0-rc.6`; no release was published.
...
1. Merge/release the runtime stack, then pin the main integration to the published
   tag. Re-run sidecar preparation against actual published assets.
