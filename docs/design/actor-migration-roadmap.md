...
**日期：** 2026-07-13  
**状态：** Implementing（S01～S05 已完成；S06～S10 pending；不得宣告 PR-4S 完成）

**范围基线：** `main @ 9886aacc750b691d6abc893808ddaaf9dfb6a538`（`fix(proxy): resolve provider-owned proxies (#4954)`；已包含 PR-4 `#4932`；S01 `daf872d9`；S02 `807f1733`；S03 工作区已验证；S04 工作区已验证：`CoreLifecycleLease` / 统一 lifecycle mutex / change_core lease span / updater stop-swap-restart）
...
| PR-2a/2b typed config actors |       ✅ |   ⚠️ 条件通过 | commit 后 mirror 失败、三域 legacy patch 部分提交、SessionState 持久化策略复核                                                                       |
...
| **PR-4S 稳定化门**           |       ⏳ | 进行中/未完成 | S01～S05 已完成；S06～S10 pending；不得宣告 PR-4S 完成。S09 的 `REGEN_BRIDGE` two-client isolation contract 是唯一稳定的 full-suite red contract。   |
...
| S-R3  | PR-2   | actor 先 `upsert`，再 mirror；mirror 失败导致“已提交但返回 Err”                                | PR-4S                                                                                                                                            |
| S-R4  | PR-2   | legacy `IVerge` patch 顺序写三个 actor，无 version check 与 compensation                       | PR-4S                                                                                                                                            |
