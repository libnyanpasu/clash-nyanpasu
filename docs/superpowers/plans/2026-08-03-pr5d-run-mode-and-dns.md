...
## 5. 五条已知问题 —— **全部已裁定**（审查者送的设计输入，不等第一轮审查再提）

### #2 C2 不可线性化 —— **已裁定**（与 #4 同一机制的两面）
...
**已裁定**：**三步同处一个 `CoreOperationGuard` 之内**，定序机制**就是 `OperationGate` 的 FIFO**——它是 5a 已实现且已测的现成机制，不需要新造。这样两次控制动作天然串行，晚到的 probe 不可能跨越守卫边界落在别人中间。

**但它直接撞上 #4 的死锁**（`reconcile()` 自己还要取一次守卫），所以两条必须一起解。

**另一半（已裁定）**：`update_service()` **只等更新进程退出、不等 daemon 就绪**，所以删掉轮询之后一次立即 probe 可能把模式**永久判成 `Normal`**（没有轮询再来纠正了）。

改为**有界等待就绪**（轮询 `status()` 直到兼容或超时），而非单次 probe。**超时后的去向裁定为 degraded，不是 `Err`**：

> **更新进程本身成功退出了。** 此时返回 `Err` 等于告诉用户「更新失败了」，而那是假的——更新完成了，只是服务还没应答。而 **Local 是一个合法的运行状态**（PR-5-pre 的 fail-closed 门本就如此设计），落到 Local 不是故障。
>
> 所以产出一条 degradation，措辞点明「**服务在 `<时限>` 内未就绪，当前以 Local 模式运行**」。与 5b 的 I-A 同源：**已经成功的事不许报成失败；没做成的后置副作用报降级。**

**时限取值与依据**：**实施时实测 daemon 从 `update_service()` 返回到 `status()` 报兼容的耗时**，取其上界并留余量；**不许拍一个「5 秒」了事**——依据必须写进实施报告。

### #3 `health_check` 的警告职责没人接手 —— **已裁定**

它不只是 status + 分类 + 循环，还**发不兼容 daemon 的警告并派发转换**——而 **smoke 2 明确要求那条警告**。

**已裁定**：探针返回值已含 `ServiceCompat`（§3.1），**由 reconcile 路径上的单一调用点发警告**——具体位置是 `CoreModeReconciler::reconcile` 内、`classify` 之前；**九处调用点都不各自发**，否则会刷屏。smoke 2 要的就是这一条。

### #4 拆 DNS 的守卫跨度与 `reconcile()` 死锁 —— **已裁定**
...
**已裁定**：引入**「已持守卫」的 reconcile 变体**（`reconcile_with(&guard, ..)`），原 `reconcile()` 保留为「自取守卫」的薄包装。这同时解决 #2 的原子性需求——**两条是同一个机制的两面**。

### #5 读路径同样不检查退出码 —— **已裁定 (a)：自己实现读**

F41：上游读路径不检查 `output.status`，空/不可解析 stdout → `Ok(None)`。**于是当原始 DNS 本就是 `None` 时，一次失败的读会与期望值「相等」，恢复被误报成功**——而 `None` **正是关闭 TUN 的主路径**。

**裁定 (a)：我们的适配器自己实现读**（自起子进程 + 检查 `ExitStatus`），不用上游 `get_dns`。
...
> **「没有配置 DNS」必须被正向识别，不能从「输出空 / 解析不了」推断。** `networksetup -getdnsservers` 在无 DNS 时输出的是**一句可识别的文字**，不是空串——**正是「把不可解析当成 `None`」这一步制造了原来的 bug**。
...
**第 4 条是关键：不认识的输出是错误，不是「没有」。** 四态各配一条断言（T-DNS-08…11，§7）。
...
>
> **本表不允许带着「提案中」进实施**——那等于把设计决策推给实施者。v2 已将三条提案全部定稿；**若后续新增行，同样必须在定稿前变成「已裁定」或被删掉**。

| 断言                                 | 靠什么机制保证                                                 | 状态                            |
| ------------------------------------ | -------------------------------------------------------------- | ------------------------------- |
| 两次控制动作的模式结论不交错         | `OperationGate` FIFO（5a 已有、已测）                          | **已裁定**（#2/#4）             |
| 恢复发生在后端动作与 reply 之前      | actor 处理器内的 `await` 点                                    | 已裁定（2.2）                   |
| 拆 DNS 发生在 stop / uninstall 之前  | **同一守卫内的调用点顺序**（守卫由 #4 的「已持守卫」变体贯穿） | **已裁定**（#4）                |
| `Shutdown` 后不再有 `SetTunDns` 生效 | 守卫准入检查（与 `ShuttingDown` 同形）                         | 已裁定（2.4）                   |
| update 之后模式反映的是 v2 daemon    | **有界等待就绪**（超时 → degraded，非 `Err`）                  | **已裁定**（#2 后半）           |
