...
**Status:** OPEN — local Full QA (WSL2) recorded in §5; all manual / true-core / multi-OS tip CI rows **PENDING**.
**Does not close PR-4S.** Local Q PASS is not tip CI and not manual smoke completion.
**Spec anchors:** `design.md` §13.2; PR-4 `#4932` five manual scenarios; `task.md` S10.
**Evidence branch / tip (at matrix revision):** `refactor/pr-4s` @ `b70aefb0`
...
These items are **implemented on the branch** (scripts / package integration / focused ledger tests + committed snapshot). They are **not** a claim that tip CI has already run green on the current SHA.
...
| Target-tip multi-OS CI run evidence for current tip     | **PENDING**     | Need durable CI URL(s) on the tip commit under test; local §5 results must be re-run/linked there                                |
...
**Honesty bound:** These PASS rows are **local** durable records for this S10 docs/working-tree state. They **must be re-run and/or linked on the final target commit and tip multi-OS CI** before PR-4S closeout. They do **not** satisfy Q-09…Q-11 or any manual `E-xxx` row.
...
| Q-09 | Target-tip CI — Windows                                                                        | —                               | CI on tip SHA       | Windows    | **PENDING**                                                                                                                                                 | none             | —                    | —          |
| Q-10 | Target-tip CI — macOS                                                                          | —                               | CI on tip SHA       | macOS      | **PENDING**                                                                                                                                                 | none             | —                    | —          |
| Q-11 | Target-tip CI — Linux                                                                          | —                               | CI on tip SHA       | Linux      | **PENDING**                                                                                                                                                 | none             | —                    | —          |
...
| Final target commit re-run on clean tip SHA                                       | **Still required** (these rows are not tip-locked) |
| Multi-OS tip CI (Q-09…Q-11)                                                       | **PENDING**                                        |
...
| Architecture ledger gate + snapshot + lint hook + focused tests | S10 tooling | A (tooling) | Implemented; local Q-01/Q-02 PASS; tip CI PENDING (§2, §5) |
...
Local Q rows use in-document result strings as durable artifacts. Manual `E-xxx` and tip CI rows still require external artifacts before PASS.
...
- [ ] Local results re-run/linked on final target commit
- [ ] Target-tip multi-OS CI evidence attached (Q-09…Q-11)
- [ ] No PASS without artifacts (local Q: in-document; E/tip CI: still missing)
- [ ] A proxies not substituted for M records

**Current verdict:** local Full QA **recorded**; multi-OS tip CI + manual smoke **incomplete**. PR-4S / S10 remain **OPEN**.
