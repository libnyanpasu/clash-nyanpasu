...
| `rebuild_running_config`（**后台脏重建**，`RebuildCoordinator` worker）                                                      | 无（响应更早的 commit）       | 除 `RuntimeRevisionExhausted` 不变量错误外全部经 sink 发布——commit 早已发生，此处只是迟到的副作用；bare `Err` 映射 `core_operation_unavailable`，见 §2.4 |
...
| **② 后台 worker**：`RebuildCoordinator` 的 dirty 重建                                                                             | 有（更早）      | **`CoreDegradationSink::publish`**（F29 已有的注入面，与 5a 的 `core_recovery_exhausted` 同一 sink）；既有 degradation 与 bare `Err` 都必须发布，bare `Err` 映射为 `core_operation_unavailable` | 已覆盖                    |
...
内部：rebuild_pipeline(..) -> (Result<RuntimeApplyReport>, Vec<Degradation>)   ← 只报事实，不决定表现形式
  ...
  ├─ ② 后台 worker      → 逐条 degradation.publish()；bare Err 映射为
  │                       core_operation_unavailable 后 publish
...
> §2.3 的 E-a/E-b 在有 caller 的 mutation 上仍返回普通 `Err`。后台 worker 没有 caller，不能把 bare `Err` 只留在日志中；除类型化的 `RuntimeRevisionExhausted` 继续作为 E-b 响亮失败外，其余 guard/pipeline `Err` 必须经 sink 发布 `core_operation_unavailable`。
...
| T-PC-09  | 后台 rebuild 失败                                  | `RebuildCoordinator` worker                    | **degradation 到达 `CoreDegradationSink`**，而不是只进日志；覆盖三类：管线已产 degradation、guard acquisition bare `Err`、pipeline bare `Err`。后两类映射 `CoreLifecycle` / `core_operation_unavailable`；同一普通失败经**同步** caller 时出现在 `MutationOutcome` 里                                                                                                  |
| T-PC-13  | revision allocator 耗尽                            | allocator 单测                                 | 返回类型化 `RuntimeRevisionExhausted`；作为 §2.3 E-b 普通 `Err`，不得映射 degradation                                                                                                                                                                                                                                                                                  |
