...
impl NyanpasuClient {
    ...
    async fn regenerate_runtime_with(
        ...
        let (state, legacy_runtime, yaml) = tokio::task::spawn_blocking(
            move || -> anyhow::Result<(
    ...
    fn failed_check_keeps_runtime_state_unpublished() {
        ...
        tauri::async_runtime::block_on(async {
            let uid = client
                ...
                .await
                .expect("add");
            // At T4 a failed rebuild still bubbles Err (the degraded model arrives in T8).
