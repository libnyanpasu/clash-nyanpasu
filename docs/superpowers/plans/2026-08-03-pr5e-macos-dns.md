...
| `stop(None)` 的撤回依赖 `CoreClientInner::drop` 是否在退出时触发，未确立 | **已核实并向前推了一步**：新增 **§9.4**——该问题的**两个分支都不救 S3**，只剩一条机会主义路径。R-C3-6 措辞从"可能没跑"收紧为"不会执行"，且**不以 5d v8 的结论为前提**                                 |
...
| **R-C3-6** | **关停超时 ⇒ S3 恢复不会执行**（措辞按 §9.4 收紧：不是"可能没跑"，而是"跑到属于机会主义，三个条件缺一即不跑"）。5d 已撤回 `stop(None)`，且**不留 facade 侧 S3 槽位**，因此**没有第二次机会**    | **v2 新增；5d R-C2-4 在本 PR 的对应面。** 其"慢、随后返回则 `Shutdown` 照常执行"的前提**尚未确立**，见 §9.4                     | 用户可见后果与指引同 R-C3-1。移除条件 = PR-6 启动时清理；或后端 await 各自有界（5a 设计面）。**不以 5d v8 解决 `Drop` 问题为条件**——§9.4 证明两个分支都不救 S3    |
...
### 9.4 关停超时后 S3 到底会不会跑 —— **三个条件的合取，没有一个被保证**

5d v7 §4.6.3 用一张逐案表论证撤回 `stop(None)` 更优，其中「处理器只是**慢**、随后返回」那一格写的是：
...
| (b) 它在**进程退出之前**返回                           | **未确立。** 生产入口是 `utils/help.rs:249` 的 `cleanup_processes`：`:263` `client.shutdown().await` 返回后，还有 `:265` `widget_manager.stop().await` 与 `:270`，随后 `quit_application`（`:274`）调 `app_handle.exit(0)`。**这个窗口存在但长度不受任何约束**，也没有任何机制让它等 actor                                                                                    |
| (c) 在此期间**没有** `stop(None)` 抢在 `Shutdown` 之前 | **未确立，且与 (b) 反向拉扯。** `CoreClientInner::drop` 就是 `self.actor_ref.stop(None)`（**已核实 `client/core.rs:377-380`**）。`cleanup_processes` 在 `:258-260` 持有的是一个 `clone()`；它一旦成为最后一个引用而被 drop，`stop(None)` 就落到**优先级更高的 stop 端口**上（5d §4.6.3 已核实 ractor 的 `listen_in_priority` 顺序），**抢在排队的 `Shutdown` 之前终止 actor** |

**结论——对本 PR 而言，`Drop` 那个悬而未决的问题不必等 5d v8 解决：**

- 若 `CoreClientInner::drop` **会**在退出路径上触发 ⇒ `stop(None)` 抢占 ⇒ **S3 不跑**（这正是 5d 撤回 `stop(None)` 要避免的那一格，只是这次由 `Drop` 送来）；
- 若它**不会**触发 ⇒ 没有任何东西终止 actor，进程随后 `exit(0)` ⇒ **S3 也不跑**；
- **只有第三种情形 S3 会跑**：处理器在 (b) 的窗口内返回、且 (c) 尚未发生。**那是机会主义，不是保证**，且窗口长度、`Drop` 时机两者都不可控。

**因此 R-C3-6 的措辞按"不会执行"写，而不是"可能没跑"。** 本 PR 不把任何正确性挂在这三条上；用户可见的兜底是 R-C3-1 的手动指引，机制上的兜底是 PR-6 的启动时清理。
...
| **关停超时后 S3 不执行**，且该结论**不依赖** 5d v8 对 `CoreClientInner::drop` 的裁定               | §9.4 的三条件核实；R-C3-6 措辞已按此收紧。**回递给 5d 的一条措辞修正也在 §9.4 末**                                                                                                                                         |
